...
public class Client
{
    ...
    {
        File clientadd=new File("src/Banking2/customerss.txt");
        try
        {
            ...
            while(scn.hasNextLine())
            {
                ...
                ss= s[1].split("");
                long NLine=-1;
                try(LineNumberReader linenmr= new LineNumberReader(new FileReader(new File("src/Banking2/customerss.txt"))))
                {
    ...
    void intofile()
    {
        try
        {
            FileWriter fos=new FileWriter("src/Banking2/customerss.txt",true);
